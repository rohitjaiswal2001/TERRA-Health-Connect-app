...
**When the member declines the categories.** iOS never tells an app which Apple
Health *read* permissions were granted — a declined type and an empty one both
just return no data, by design. So `getGivenPermissions()` isn't reliable (it
isn't even implemented on iOS in this SDK version, and the app falls back to the
requested set). The one honest signal is **whether any data actually arrived**:
`syncAll()` returns a [`SyncOutcome`](lib/services/terra_service.dart)
(`pushed`/`total`), and when `pushed == 0` — nothing came back from a single
category — the flow routes to [`no_data_screen.dart`](lib/screens/no_data_screen.dart)
(`ConnectionPhase.noData`) instead of the celebratory connected screen. That
screen states it plainly, spells out `Settings › Health › Data Access & Devices ›
Personally` (the only place iOS lets them change it — there's no deep link into
it), and offers a **Re-sync** that promotes them to connected the moment data
flows. A *partial* decline is deliberately treated as success: iOS can't tell us
which types were denied, and "no heart data" is usually a legitimate absence, not
a refusal — flagging it would cry wolf on every launch.
